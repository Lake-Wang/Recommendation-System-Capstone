...
def compute_gender_direction(data, S, user_vectors):
    ageEmbed = np.zeros((2,user_vectors.shape[1]))
    # S = 0 indicates male and S = 1 indicates female
    num_users_per_group = np.zeros((2,1))
    for i in range(len(data)):
        u = data['user_id'][i]
        if S['binary_age_group'][i] == 0:
            ageEmbed[0] +=  user_vectors[u]
            num_users_per_group[0] += 1.0
        else:
            ageEmbed[1] +=  user_vectors[u]
            num_users_per_group[1] += 1.0
    
    ageEmbed = ageEmbed / num_users_per_group # average gender embedding
    return ageEmbed

def compute_bias_direction(age_vectors):
    age_vBias= age_vectors[1].reshape((1,-1))-age_vectors[0].reshape((1,-1))
    age_vBias = age_vBias / np.linalg.norm(age_vBias,axis=1,keepdims=1)
    return age_vBias

def linear_projection(data,user_vectors,age_vBias):
    # linear projection: u - <u,v_b>v_b
    for i in range(len(data)):
        u = data['user_id'][i]
        user_vectors[u] = user_vectors[u] - (np.inner(user_vectors[u].reshape(1,-1),age_vBias)[0][0])*age_vBias
    return user_vectors
#%% load data
...
debiased_NCF.load_state_dict(torch.load("trained-models/preTrained_NCF", map_location='cpu'))
...
age_embed = compute_gender_direction(train_data, train_protected_attributes, users_embed)
np.savetxt('results/age_embed.txt',age_embed)

age_vBias = compute_bias_direction(age_embed)
np.savetxt('results/age_vBias.txt',age_vBias)

# incorporate all users: debias train & test both
all_data = (pd.concat([train_data,test_data],axis=0)).reset_index(drop=True)
age_debias_users_embed = linear_projection(all_data,users_embed,age_vBias) # first debias training users
#debias_users_embed = linear_projection(test_data,debias_users_embed,vBias) # then debias test users
np.savetxt('results/age_debias_users_embed.txt',age_debias_users_embed)
