...
def compute_bias_direction(gender_vectors):
    gender_vBias= gender_vectors[1].reshape((1,-1))-gender_vectors[0].reshape((1,-1))
    gender_vBias = gender_vBias / np.linalg.norm(gender_vBias,axis=1,keepdims=1)
    return gender_vBias

def linear_projection(data,user_vectors,gender_vBias):
    # linear projection: u - <u,v_b>v_b
    for i in range(len(data)):
        u = data['user_id'][i]
        user_vectors[u] = user_vectors[u] - (np.inner(user_vectors[u].reshape(1,-1),gender_vBias)[0][0])*gender_vBias
    return user_vectors
#%% load data
...
gender_vBias = compute_bias_direction(gender_embed)
np.savetxt('results/gender_vBias.txt',gender_vBias)

# incorporate all users: debias train & test both
all_data = (pd.concat([train_data,test_data],axis=0)).reset_index(drop=True)
gender_debias_users_embed = linear_projection(all_data,users_embed,gender_vBias) # first debias training users
#debias_users_embed = linear_projection(test_data,debias_users_embed,vBias) # then debias test users
np.savetxt('results/gender_debias_users_embed.txt',gender_debias_users_embed)
